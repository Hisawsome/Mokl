...
def make_request(auth_token):
    ...
    # Generate a random tap count
    tap_count = random.randint(270, 330)
    ...
    try:
        ...
        if response.status_code == 200:
            print(f"[✔] Success for token: {auth_token}. Tapped {tap_count} times.")
        else:
...
def main():
    ...
    while True:
        ...
        for token in authorizations:
            make_request(token)
            time.sleep(random.randint(2, 5))  # Add a random delay between requests
