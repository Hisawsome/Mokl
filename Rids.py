...
def get_balance(auth_token):
    """Get the current balance for the provided token."""
    headers = HEADERS_TEMPLATE.copy()
    headers["authorization"] = f"Bearer {auth_token}"

    try:
        response = requests.get("https://api.mokl.io/public/api/clicker/balance", headers=headers)
        if response.status_code == 200:
            balance_data = response.json()
            return balance_data.get("balance", 0)
        else:
            print(f"[✖] Failed to get balance for token: {auth_token}, Status: {response.status_code}, Response: {response.text}")
            return None
...
def make_request(auth_token):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Fetch and write balance and last tapped time
            balance = get_balance(auth_token)
            if balance is not None:
                print(f"[✔] Current balance for token: {auth_token}: {balance}")
                
                # Log the balance and last tapped time
                with open("tap_log.txt", "a") as log_file:
                    log_file.write(f"Token: {auth_token}, Balance: {balance}, Last Tapped Time: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(timestamp))}\n")
            else:
